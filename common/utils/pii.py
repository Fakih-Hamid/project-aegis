...
DEFAULT_PATTERNS: Sequence[PiiPattern] = (
    PiiPattern(label="email", pattern=re.compile(r"[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}"), replacement="<redacted-email>"),
    PiiPattern(label="api_key", pattern=re.compile(r"(?:sk|pk|key)_[A-Za-z0-9]{16,}"), replacement="<redacted-api-key>"),
    PiiPattern(label="phone", pattern=re.compile(r"\+?\d[\d\s\-]{7,}\d"), replacement="<redacted-phone>"),
    PiiPattern(label="credit_card", pattern=re.compile(r"\b(?:\d[ -]*?){13,16}\b"), replacement="<redacted-cc>"),
    PiiPattern(label="secret", pattern=re.compile(r"secret=([A-Za-z0-9+/=]{8,})", re.I), replacement="secret=<redacted>"),
)
